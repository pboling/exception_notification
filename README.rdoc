...
The Super Exception Notifier gem provides a mailer object and a default set of
...
This gem, Super Exception Notifier, is based on the wonderful exception_notification plugin
...
just set any or all of the following values:
  ...
  ExceptionNotifier.sender_address = %(MYRAILSAPP #{(defined?(Rails) ? Rails.env : RAILS_ENV).capitalize} Error <errors@example.com>)

  # defaults to "[ERROR] "
  ExceptionNotifier.email_prefix = "[APP-#{(defined?(Rails) ? Rails.env : RAILS_ENV).capitalize} ERROR] "
...
Super Exception Notifier allows you to specify the layout for errors at several levels:
  * all errors use same layout site-wide
  * customize just in a particular controller
  * can use the same layout as the controller
  * no layout at all

By default it will render the error with the layout the controller is using. You just need to set in application.rb (or per-controller):
...
== HTTP Error Codes Used by Exception Notifier by default
  ...
  418 IÕm a teapot
    * ErrorDocument IÕm a teapot | Sample 418 IÕm a teapot
  ...
  422 Unprocessable Entity
    ...
    * (WebDAV) (RFC 4918 ) Ð The request was well-formed but was unable to be followed due to semantic errors.
  423 Locked
    * ErrorDocument Locked | Sample 423 Locked
    * (WebDAV) (RFC 4918 ) Ð The resource that is being accessed is locked
  500 Internal Server Error
